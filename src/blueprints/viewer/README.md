...
- Part outline groups (Walls / Roof / Interior) with per-group and per-leaf **opacity** sliders (tap toggles 0 ↔ last value). Translucent parts use multi-pass **depth peeling** (linear eye-space Z; hardware depth is not sampled — incompatible with `logarithmicDepthBuffer`) so faded layers composite correctly through each other while unfaded solids stay true occluders. Opacity 0 hides a part (also omitted from AR). When any part is faded the frame uses the peel path (extra passes); opaque colour/background should still match the normal path.
