...
- **Custom scenes** (optional, model-specific): extra View buttons after the builtins. Each scene can set camera pose (`position` / `target` / optional `up`), optional FOV or orthographic ISO (`projection: "ortho"`), optional section cuts, and optional part opacities (`opacity` + `opacityDefault`). Loaded from `docs/models/<id>.scenes.json` (Pages) or embedded in the offline HTML export. Coordinates match the GLB (CAD metres: X eave↔eave, Y kitchen↔living, Z up).
