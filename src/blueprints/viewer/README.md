...
- **Edges** toggle (Materials sheet): CAD hard-edge overlay via fat `LineSegments2` strokes (black, ~3.5px) plus section-cut silhouettes. Always drawn in a post-face depth-aware pass (never mixed into the face pass — coplanar log-depth fights hide edges). FragDepth bias keeps strokes on top of their faces. While depth-peeling, the same pass runs after composite so faded parts keep outlines. Persists in `localStorage`. Off by default. Overlays are omitted from AR and Measure picking.
