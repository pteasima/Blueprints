...
- **Edges** toggle (Materials sheet): CAD hard-edge overlay via fat `LineSegments2` strokes (black, ~3.5px). Persists in `localStorage`. Off by default. Overlays are omitted from AR and Measure picking; hidden during depth-peel override / transparent passes so mesh shaders are not applied to lines.
