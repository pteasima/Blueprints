...
- **Edges** toggle (Materials sheet): CAD hard-edge overlay via `EdgesGeometry` (diagrammatic line colours from `SOLID_LINE_COLORS`). Persists in `localStorage`. Off by default. Overlays are omitted from AR and Measure picking; hidden during depth-peel override / transparent passes so mesh shaders are not applied to lines.
