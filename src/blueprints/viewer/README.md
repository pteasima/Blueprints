...
- **Edges** toggle (Materials sheet): CAD hard-edge overlay via fat `LineSegments2` strokes (black, ~3.5px) plus section-cut silhouettes. FragDepth bias keeps edges visible in ISO (log-depth makes polygonOffset a no-op). While depth-peeling, edges are drawn in a post-composite depth-aware pass so faded parts keep their outlines. Persists in `localStorage`. Off by default. Overlays are omitted from AR and Measure picking.
