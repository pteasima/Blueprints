...
- **Parts** outline (iOS-style disclosure groups, collapsed by default): Walls / Roof / Interior (recursive group tree). Opacity sliders on groups and leaves (0 = hidden; tap toggles 0 ↔ last non-zero). Group slider is a proxy for its leaves. When any part is faded, all visible meshes use weighted blended OIT; meshes still at 100% are rendered at α=0.99 for that frame only so they stay on the all-translucent path (true α=1 inside WBOIT broke occlusion). Camera presets (Iso / Front / Side / Top)
