...
- Part outline groups (Walls / Roof / Interior) with per-group and per-leaf **opacity** sliders (tap toggles 0 ↔ last value). Translucent parts use standard Three.js alpha (`transparent` + `depthWrite:false`, painter’s algorithm). Optional Everitt **depth peeling** lives in `depthPeel.js` behind `USE_DEPTH_PEEL` (default **off** — peels hid faded parts when view-Z / stage inject failed). Opacity 0 hides a part (also omitted from AR).
