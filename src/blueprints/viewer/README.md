...
- **Parts** outline (iOS-style disclosure groups, collapsed by default): Walls / Roof / Interior (recursive group tree). Opacity sliders on groups and leaves (0 = hidden; tap toggles 0 ↔ last non-zero). Group slider is a proxy for its leaves. When any part is faded, all visible meshes use weighted blended OIT (solids as α=1) so fades composite without depth-split punch-through. Camera presets (Iso / Front / Side / Top)
